...
def main(args):
    ...
    train_unprocessed_tsv_path = os.path.join(
        input_train_dir, "train_unprocessed.tsv")
    dev_unprocessed_tsv_path = os.path.join(
        input_dev_dir, "dev_unprocessed.tsv")
    ...
    logging.info(
        f"There are {len(unique_train_questions)} unique questions in train")
    logging.info(
        f"There are {len(unique_dev_questions)} unique questions in dev")

    train_df["bad_candidate"] = train_df["answerEntity"].apply(
        lambda x: True if x.startswith('P') else False)
    train_df = train_df[~train_df["bad_candidate"]]
    dev_df["bad_candidate"] = dev_df["answerEntity"].apply(
        lambda x: True if x.startswith('P') else False)
    ...
    logging.info(
        f"Train size after no-entity questions drops: {train_df.shape}")
    ...
    wikidata_id2name.update(
        create_wikidata_id2name_map(dev_df["graph"].values))
    ...
    train_df["answerEntity"] = train_df["answerEntity"].apply(
        lambda x: wikidata_ids2names(x, wikidata_id2name))
    logging.info("Mapping train questionEntity wikidata ids to labels")
    train_df["questionEntity"] = train_df["questionEntity"].apply(
        lambda x: wikidata_ids2names(x, wikidata_id2name))
    logging.info(
        "Mapping train groundTruthAnswerEntity wikidata ids to labels")
    ...
    dev_df["answerEntity"] = dev_df["answerEntity"].apply(
        lambda x: wikidata_ids2names(x, wikidata_id2name))
    logging.info("Mapping dev questionEntity wikidata ids to labels")
    dev_df["questionEntity"] = dev_df["questionEntity"].apply(
        lambda x: wikidata_ids2names(x, wikidata_id2name))
