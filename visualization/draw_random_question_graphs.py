...
def main(args):
    ...
    for i, q in enumerate(random_qs):
        ...
        for _, row in ddf.iterrows():
            ...
            true_s = row["groundTruthAnswerEntity"].replace(
                '/', '|').replace('\\', '|')
            ...
            for n_dict in graph_json["nodes"]:
                ...
                labels[n_id] = n_label
                color = color_map[n_type] if color_map.get(
                    n_type) is not None else "#808080"
                node_colors[n_id] = color

            node_colors_np = [node_colors[key]
                              for key in sorted(node_colors.keys())]
            ...
            plt.title(split_node_labels(q, max_line_length=64,
                      min_line_length=16), fontsize=12)
            ...
            nx.draw_networkx_labels(
                nx_graph, pos_node_labels, labels, font_size=8)
            ...
            output_graph_path = os.path.join(
                output_dir, f"question_{i}/", fname)
